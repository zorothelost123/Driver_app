import { ArrowLeft, Clock3, MapPin, Navigation, Route, Sparkles, TrendingUp, Zap } from "lucide-react";
...
export function SurgPage() {
  ...
  const selectedRoute = routeOptions.find((option) => option.id === routeOption) ?? routeOptions[0];
  ...
  return (
      ...
      header={
        <div className={`${surfaceCardClassName} overflow-hidden`}>
          ...
          >
            ...
            <div className="absolute -right-10 bottom-2 size-32 rounded-full bg-white/10 blur-3xl" />
            ...
            <div className="relative mt-6 grid grid-cols-3 gap-3">
              ...
              <div className="rounded-[22px] border border-white/15 bg-white/10 p-4">
                ...
                <p className="mt-2 text-base font-semibold">{bestZone.waitTimeLabel}</p>
              </div>
              <div className="rounded-[22px] border border-white/15 bg-white/10 p-4">
                <p className="text-xs uppercase tracking-[0.2em] text-white/60">
                  Forecast
            ...
            <div className="relative mt-4 rounded-[24px] border border-white/15 bg-white/10 p-4 backdrop-blur-md">
    ...
    >
      ...
      >
        <div className="relative h-[330px] overflow-hidden rounded-[24px] bg-[linear-gradient(180deg,#edf4ff_0%,#def7f0_100%)] p-4">
          ...
          {demandZones.map((zone, index) => {
            ...
            return (
              ...
              >
                ...
                />
                <div
                  className="absolute left-1/2 top-1/2 flex -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-white/85 p-2 shadow-sm"
                >
          ...
          <div className="absolute left-4 top-4 rounded-[18px] border border-white/80 bg-white/88 px-4 py-3 shadow-sm backdrop-blur">
          ...
          <div className="absolute bottom-4 left-4 rounded-[20px] border border-white/80 bg-white/92 p-3 shadow-sm backdrop-blur">
            <div className="space-y-2 text-xs font-medium text-slate-600">
              {(["high", "medium", "low"] as const).map((level) => (
                <div key={level} className="flex items-center gap-2">
                  <span className={`size-2.5 rounded-full ${demandStyles[level].dot}`} />
      ...
      {avoidTrafficMode ? (
        <motion.section
          initial={{ opacity: 0, y: 18 }}
          animate={{ opacity: 1, y: 0 }}
          className={`${softCardClassName} p-5`}
        >
          <div className="flex items-start gap-3">
            <div className="rounded-[18px] bg-emerald-600 p-3 text-white">
            ...
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-slate-500">
                Traffic-heavy corridors are being deprioritized. You will be
                guided through calmer stretches to preserve pickup speed.
              </p>
            </div>
          </div>
        </motion.section>
          ...
          return (
            ...
            >
              ...
              <div className="mt-4 rounded-[22px] border border-slate-200 bg-slate-50/80 p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      {selectedRoute.etaLabel} • {selectedRoute.fuelLabel}
                    </p>
                  </div>
              ...
              <div className="mt-4 flex items-center justify-between gap-3">
                ...
                <button className="inline-flex items-center gap-2 rounded-full bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-800">
