import { ArrowLeft, CalendarDays, ChevronRight, Clock3, Landmark, TrendingUp, Wallet } from "lucide-react";
import { motion } from "motion/react";
import { useNavigate } from "react-router";
import { AppShell } from "../components/AppShell";
import { useDemoAppContext } from "../context/DemoAppContext";
import { earningsSummaries, recentTrips, weeklyEarnings } from "../lib/demo-data";
  ...
  return (
    ...
    >
      <motion.section
        ...
        className={`${surfaceCardClassName} overflow-hidden`}
      >
        <div className={`p-6 text-white ${brandGradientClassName}`}>
          ...
          <div className="mt-6 grid grid-cols-3 gap-3">
            ...
            <div className="rounded-[22px] border border-white/15 bg-white/10 p-4">
              ...
              <p className="mt-2 text-lg font-semibold">{summary.onlineHours}h</p>
            </div>
            <div className="rounded-[22px] border border-white/15 bg-white/10 p-4">
              <p className="text-xs uppercase tracking-[0.2em] text-white/60">
                Avg / trip
          ...
          {weeklyEarnings.map((entry, index) => {
            ...
            return (
              <div key={entry.day} className="flex flex-1 flex-col items-center">
                ...
                <p className="mt-3 text-xs font-medium text-slate-500">{entry.day}</p>
              </div>
            );
          })}
        </div>
      ...
      <section className={`${surfaceCardClassName} p-5`}>
        <div className="flex items-center justify-between">
          ...
          <button className="text-sm font-semibold text-sky-700">View all</button>
        </div>
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm font-semibold text-slate-950">{trip.from}</p>
                  </div>
                  <div className="mt-2 flex items-center gap-2">
                    <span className="size-2 rounded-full bg-rose-400" />
                    <p className="text-sm font-semibold text-slate-950">{trip.to}</p>
                  </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-500">
              Earnings will be transferred to your bank account after the final
              review cut-off at 9 AM.
            </p>
